...
export default function AddPaymentDialog({
  ...
  setClient,
  client,
  presetAmounts = [2400, 22000, 40000],
  fetchDataPayList,
}) {
  ...
  const handleAddPayment = async () => {
    if (client && amount && paymentDate) {
      ...
      const newPayment = {
        ...
        customPaymentType: customPaymentType !== '' ? customSessionsCount + ' ' + customPaymentType : customPaymentType,
          ...
          {paymentType === "Другое" && (
            <Box sx={{ mt: 2, display: "flex", gap: "2px" }}>
              <TextField
                ...
                placeholder="Корпоративный пакет и т.д."
              />
              <TextField
                ...
                onChange={(e) => {
                  ...
                  setCustomSessionsCount(value >= 1 ? value : 1);
                }}
                inputProps={{ min: 0 }}
                placeholder="Например: 8, 12, 20"
              />
            </Box>
          )}
        </Box>
                  ...
                  <TextField
                    ...
                    helperText={
                      !isExpiryDateManuallySet
                        ? "По умолчанию: 6 месяцев с даты оплаты"
                        : ""
                    }
                  />
                )}
